      ...
      <section className="space-y-4 rounded-2xl border border-border bg-card p-5 shadow-sm">
        ...
        <div className="space-y-2 rounded-xl border border-blue-100 bg-blue-50 p-3">
          ...
          {file ? <div className="flex gap-1.5 overflow-x-auto pb-1">
            ...
            {mergeClips.map((clip, index) => <div key={clip.id} className={`relative min-w-28 rounded-lg border ${selectedClipId === clip.id ? 'border-primary bg-white ring-1 ring-primary' : 'border-blue-100 bg-white/70'}`}><button type="button" onClick={() => selectQueuedClip(clip)} className="block w-full px-2.5 py-2 text-left"><span className="block text-[9px] font-bold text-primary">{String(index + 2).padStart(2, '0')} · CLIP</span><span className="block truncate pr-4 text-[10px] font-medium" title={clip.file.name}>{clip.file.name}</span>{clip.reverse ? <span className="text-[9px] text-violet-600">Reversed</span> : null}</button><button type="button" onClick={() => removeMergeClip(clip.id)} aria-label={`Remove ${clip.file.name}`} className="absolute right-1.5 top-1.5 rounded p-0.5 text-slate-400 hover:bg-red-50 hover:text-red-600"><Trash2 className="h-3 w-3" /></button></div>)}
          </div> : null}
